...
Centrality measures are characteristics of nodes showing their *importance* in various aspects. In this section, we will identify nodes on the network with the highest centrality in the following measurements: degree centrality, closeness centrality, and betweenness centrality.
...
The origin constrained model is used for the analysis to preserve the number of commuters starting their journeys in each area. We will observe how the destination of commuters changed in reaction to the decrease in jobs in Canary Wharf.

We have observed a significant drop in the number of 


#### Scenario B

We will consider 2 scenarios for the change in the cost of transport. The parameters $\beta_1, \beta_2$ for the two scenarios B1 and B2 will be modified as follows.
...
![Distance decay function for the original scenario and the two modified scenarios.](img/dist_decay.png)
