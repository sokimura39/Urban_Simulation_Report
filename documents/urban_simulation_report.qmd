...
**Betweenness centrality** of a nodeis the number of shortest paths that run through. The betweenness centrality $x_i$ for node $i$ is calculated as 
...
and $g_{st}$ is the total number of geodesic paths from $s$ to $t$. When normalised by dividing by $(n-1)(n-2)/2$ (combinations between remaining nodes). High betweenness centrality on the underground shows it is on the shortest path between many pairs of stations, therefore many people are expected to pass it. The stations with the highest betweenness centrality are shown in @tbl-betweenness.
...
**Closeness centrality** is the inverse of the mean geodesic distance from one node to other reachable $n_0$ nodes ($l_i$), scaled by the ratio of reachable nodes [@wasserman1994]. The closeness centrality $C_i$ is calculated as 

$$
C_i = \frac{n_0-1}{n-1}\frac{1}{l_i} = \frac{n_0-1}{\sum_{j} d_{ij}}
$$ {#eq-closeness-centrality}

where $d_{ij}$ is the geodesic distance between nodes $i$ and $j$.

A high closeness centrality indicates the station is within a short distance from all the other stations. These stations are assumed to be located within the city centre, as the high-ranked stations clustered in central London shown in @tbl-closeness.
...
: Stations with the highest closeness centrality. Charing Cross is where the 'distance to London' is measured from [@leatherdale2016] - supporting that these stations are located in the city centre. {#tbl-closeness tbl-colwidths="[10,40,50]"}
...
This can be applied to any transport network - closures of NYC subway stations after 9/11 [@wyatt2002; @paaswell2012] is one applicable example outside London. It is also applicable for other networks as well to assess the connectivity and the performance of the network.
...
Node removal based on the betweenness centrality has the largest impact on performance. Since a high betweenness centrality indicates high flow through the node, removing this node will force a large number of flows to divert using different routes. Interestingly, the degree centrality has identified the first few stations with larger impact, and further investigation is required to identify whether this is a general trend or specific to the Underground network.
...
In this section, a weighted network considering the passenger flows is analysed. Based on the morning peak origin-destination data from @transportforlondon2021, flows have been assigned to the shortest path between nodes considering the length of each edge. 
...
Comparing the weighted degree and betweenness centralities in @tbl-weighted-centrality-measures, their difference lies in whether it accounts for journeys that originate or terminate at the node in question, and whether it double-counts flows that pass through the node. Thus, their relationship can be expressed as  
...
Based on the above measures, we now examine the stations with high weighted centrality measures.
...
We will consider the need for adjusting the impact measures in @tbl-delta-performance for the weighted network. When considering the weight, each measure should be adjusted as summarised in @tbl-delta-performance-weighted.
...
The best performing methodology in @sec-weighted is the sequential removal using betweenness centrality. Using the unweighted and weighted variants, the sequential removal removes stations shown in @tbl-node-removal-betweenness.
